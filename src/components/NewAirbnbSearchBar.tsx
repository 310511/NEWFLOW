...
import { DestinationPicker, DatePicker, GuestSelector, SearchButton } from "./NewSearchComponents";

interface Props {
  isSticky?: boolean;
}

const NewAirbnbSearchBar = ({ isSticky = false }: Props) => {
  ...
  useEffect(() => {
    const handleClickOutside = (event: MouseEvent) => {
      ...
      const clickedInsideRadixPopover = !!target?.closest('[data-radix-popover-content]');

      if (clickedInsideSearch || clickedInsideRadixPopover) {
        return; // allow interacting with popovers/inputs
      }
  ...
  return (
    <div className="w-full">
      <div 
        ...
        } ${isSticky ? 'scale-90' : ''} backdrop-blur-sm mx-6 lg:mx-8`}
      >
...
export default NewAirbnbSearchBar;