...
const NewSearchResults = () => {
    ...
    return hotels.filter(hotel => {
      ...
      if (selectedFilters.length > 0) {
        ...
        return selectedFilters.some(filter => 
          ...
          (filter === 'restaurant' && hotelAmenities.includes('restaurant')) ||
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      {/* Main Content */}
      <main className="w-full pt-header" style={{ paddingTop: 'calc(var(--header-height-default) + 20px)' }}>
        {/* Search Controls Bar */}
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center space-x-4">
              ...
              <SearchFilters
                ...
                setPriceRange={(range) => setPriceRange(range as [number, number])}
              ...
              {/* Quick Filter Badges */}
              {((priceRange[0] !== 50 || priceRange[1] !== 1000) || selectedFilters.length > 0) && (
                    ...
                    <div className="p-6 space-y-4">
                      {paginatedHotels.map((hotel) => (
                        <AirbnbHotelCard
                          ...
                          variant="map"
                          onHover={handleHotelHover}
                          isSelected={selectedHotel === hotel.id || hoveredHotel === hotel.id}
                        />
                      ))}
                    </div>
                  </div>
              ...
              <div className="w-1/2 relative">
                <FakeMapView
                  hotels={filteredHotels.map(hotel => ({
                    ...hotel,
                    coordinates: hotel.coordinates || { lat: 24.7136 + (Math.random() - 0.5) * 0.1, lng: 46.6753 + (Math.random() - 0.5) * 0.1 }
                  }))}
